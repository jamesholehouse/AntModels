module BDMAnalytic
    ...
    function pm(t::Float64, BD::BDM, m::Int64, m₀::Int64)
        ...
        if m<m₀
            return convert(Float64,N)*prod(Bs[m+1:m₀])*sum([sum_elems(λ[i], t, m, m₀, pars, N, p_arr[i], q_arr[i], den_prod[i]) for i in 1:N+1])::Complex{Double64}
        elseif m==m₀
            return convert(Float64,N)*sum([sum_elems(λ[i], t, m, m, pars, N, p_arr[i], q_arr[i], den_prod[i]) for i in 1:N+1])*N::Complex{Double64}
        else
            return convert(Float64,N)*prod(As[m₀+1:m])*sum([sum_elems(λ[i], t, m₀, m, pars, N, p_arr[i], q_arr[i], den_prod[i]) for i in 1:N+1])*N::Complex{Double64}
        end
    end
